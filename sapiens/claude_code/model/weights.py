"""Utilities for loading Sapiens pretrained weights into SapiensPose3D.

The pretrain checkpoint uses no key prefix and was trained at 1024×1024 with
a CLS token.  Our model stores the ViT under ``backbone.vit.*``, runs at
384×640, and has no CLS token.  Three conversions are needed:

1. Key remapping: ``patch_embed.*`` → ``backbone.vit.patch_embed.*``, etc.
...
def load_sapiens_pretrained(
    model: nn.Module,
    ckpt_path: str,
    verbose: bool = True,
) -> None:
    """Load a Sapiens pretrain checkpoint into a SapiensPose3D model.

    Backbone weights are loaded with patch-embed expansion and pos-embed
    interpolation.  The regression head is left randomly initialised.

    Args:
        model:     A ``SapiensPose3D`` instance.
        ckpt_path: Path to ``sapiens_0.3b_epoch_1600_clean.pth``.
    ...
    if pe_key in remapped and pe_key in model_sd:
        ...
        tgt_pe  = model_sd[pe_key]        # (1, 960, D)   — no CLS token
        tgt_N   = tgt_pe.shape[1]

        src_N_no_cls = src_pe.shape[1] - 1          # 4096 = 64×64
        src_g = int(src_N_no_cls ** 0.5)

        # Infer target grid from model
        tgt_h = int(tgt_N ** 0.5) if int(tgt_N ** 0.5) ** 2 == tgt_N else None
        # For non-square grids, get grid shape from the backbone config
        try:
        ...
        if verbose:
            print(f"[weights] pos_embed: {src_g}×{src_g} → {tgt_h}×{tgt_w}  "
    ...
    if verbose:
        print(f"[weights] Loaded  {len(load_sd)} / "
              f"{sum(1 for k in model_sd if k.startswith('backbone.'))} backbone tensors")
        head_params = sum(1 for k in model_sd if k.startswith("head."))
        print(f"[weights] Head ({head_params} tensors) randomly initialised")
        if missing:
            print(f"[weights] Missing  ({len(missing)}): {missing[:5]}"
