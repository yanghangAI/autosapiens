...
Constant Decay LLRD (gamma=0.90, unfreeze_epoch=10):
...
def _build_optimizer_frozen(model, weight_decay: float, base_lr: float, gamma: float, lr_head: float):
    """
    Epochs 0-9: blocks 0-11 + embeddings frozen.
...
def main():
    ...
    # ── LLRD Optimizer (frozen lower half for epochs 0-9) ────────────────────
    ...
    for epoch in range(start_epoch, args.epochs):
        # ── Progressive unfreezing at epoch 10 ───────────────────────────────
