"""Training entry point for SapiensPose3D — idea004/design001.

Constant Decay LLRD (gamma=0.95, unfreeze_epoch=5):
- Per-block learning rates: lr_i = 1e-4 * 0.95^(23-i)
- Patch+pos embedding: lr_embed = 1e-4 * 0.95^24
...
# ── LLRD CONSTANTS ────────────────────────────────────────────────────────────
GAMMA           = 0.95
BASE_LR_BACKBONE = 1e-4  # deepest block (block 23) gets this LR
LR_HEAD         = 1e-4
NUM_BLOCKS      = 24
UNFREEZE_EPOCH  = 5
...
def _block_lr(block_idx: int) -> float:
    ...
    return BASE_LR_BACKBONE * (GAMMA ** (NUM_BLOCKS - 1 - block_idx))


def _embed_lr() -> float:
    """LR for patch+pos embedding: base_lr * gamma^24."""
    return BASE_LR_BACKBONE * (GAMMA ** NUM_BLOCKS)
...
def _build_optimizer_frozen(model, weight_decay: float):
    """
...
def _build_optimizer_full(model, weight_decay: float):
    """
...
def main():
    ...
    optimizer = _build_optimizer_frozen(model, weight_decay=args.weight_decay)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch == UNFREEZE_EPOCH:
            ...
            optimizer = _build_optimizer_full(model, weight_decay=args.weight_decay)
