"""Training script for Kinematic Attention Masking - Design 2: Soft Kinematic Mask.
...
Design 2 applies a soft additive bias to the self-attention logits of the
joint query decoder, penalizing attention between structurally distant joints.
The bias is: soft_bias[i, j] = HOP_DIST[i, j].float() * math.log(0.5)
This equals 0 at d=0 (self), log(0.5)≈-0.693 at d=1, log(0.25)≈-1.386 at d=2, etc.
The bias is applied via tgt_mask in self.decoder() at every decoder layer.
...
class _Cfg:
    ...
    output_dir  = "/work/pi_nwycoff_umass_edu/hang/auto/runs/idea002/design002"
    resume      = ""
...
def main():
    ...
    model = SapiensPose3D(
        ...
        drop_path_rate=args.drop_path, attention_method="soft_kinematic_mask",
    ).to(device)
