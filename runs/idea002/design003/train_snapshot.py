"""Training script for Kinematic Attention Masking - Design 3: Hard Kinematic Mask.
...
Design 3 applies a hard binary mask to the self-attention logits of the joint
query decoder. Joints within a 2-hop neighborhood in the kinematic tree
(including self) are allowed to attend freely (bias = 0.0); joints outside
this neighborhood are completely blocked (bias = -inf). The mask is precomputed
once, registered as a buffer, and applied at every decoder layer via tgt_mask.
No warmup or annealing; hard masking is applied from epoch 0.
...
from infra import (
    ...
    SMPLX_SKELETON,
    FLIP_PAIRS, RGB_MEAN, RGB_STD, DEPTH_MAX_METERS, FRAME_STRIDE,
...
# ── KINEMATIC HOP-DISTANCE MATRIX ────────────────────────────────────────────
# Shared module-level constant used by Designs 2 and 3 (defined here for all).
...
class BedlamFrameDataset(Dataset):
    ...
    def __init__(self, seq_paths, data_root, transform=None,
                 depth_required=True, frame_stride=FRAME_STRIDE):
...
def _worker_init_fn(worker_id: int) -> None:
    """Seed each DataLoader worker deterministically (global seed = 2026)."""
    seed = 2026 + worker_id
    random.seed(seed)
    np.random.seed(seed)
...
class Pose3DHead(nn.Module):
    ...
    Args:
        attention_method: one of "dense", "soft_kinematic_mask", "hard_kinematic_mask".
            For Design 1 ("dense"), tgt_mask=None is passed — PyTorch default, no masking.
    """
    ...
    def __init__(self, in_channels, num_joints=NUM_JOINTS, hidden_dim=256,
        ...
        # Soft kinematic mask: precompute additive logit bias from HOP_DIST
        if attention_method == "soft_kinematic_mask":
            LOG_HALF = math.log(0.5)  # ≈ -0.6931
            soft_bias = HOP_DIST.float() * LOG_HALF  # (70, 70), all finite floats
            self.register_buffer("soft_bias", soft_bias)
        # Hard kinematic mask: precompute binary float mask from HOP_DIST
        if attention_method == "hard_kinematic_mask":
            HOP_RADIUS = 2
            d = HOP_DIST  # (70, 70), dtype=torch.long
            allowed = (d <= HOP_RADIUS)  # True where hop distance <= 2
            # NaN guard: rows that are entirely False fall back to dense attention
    ...
    def forward(self, feat: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        # Design 2: soft kinematic mask — apply additive logit bias via tgt_mask
        if self.attention_method == "soft_kinematic_mask":
            out = self.decoder(queries, memory, tgt_mask=self.soft_bias)
        # Design 3: hard kinematic mask — block attention outside 2-hop neighborhood
...
class SapiensPose3D(nn.Module):
    ...
    def __init__(self, arch="sapiens_0.3b", img_size=(640, 384), num_joints=NUM_JOINTS,
        ...
        self.head = Pose3DHead(in_channels=SAPIENS_ARCHS[arch]["embed_dim"],
                               ...
                               dropout=head_dropout, attention_method=attention_method)
...
class _Cfg:
    ...
    output_dir  = "/work/pi_nwycoff_umass_edu/hang/auto/runs/idea002/design003"
    resume      = ""
...
def main():
    ...
    # ── Deterministic seeding (hardcoded to 2026 per proxy design rules) ─────
    _SEED = 2026
    random.seed(_SEED)
    ...
    print(f"  Global random seed set to {_SEED}")
    ...
    model = SapiensPose3D(
        ...
        drop_path_rate=args.drop_path, attention_method="hard_kinematic_mask",
    ).to(device)
    ...
    # Print final body MPJPE as single float to stdout (proxy eval result)
    final_body_mpjpe = val_m["val_mpjpe_body"] if val_m else float("nan")
    print(f"FINAL_VAL_MPJPE_BODY={final_body_mpjpe:.4f}")
