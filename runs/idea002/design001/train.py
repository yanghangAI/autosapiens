"""Training script for Kinematic Attention Masking - Design 1: Baseline Dense (Control).
...
Design 1 uses dense (unconstrained) self-attention in the Pose3DHead decoder.
It introduces a unified Pose3DHead class with an `attention_method` argument,
and defines HOP_DIST at module level for use by Designs 2 and 3.
...
class Pose3DHead(nn.Module):
    ...
    def forward(self, feat: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        # Design 1: dense attention — pass tgt_mask=None (PyTorch default, no masking)
        out = self.decoder(queries, memory, tgt_mask=None)  # (B, num_joints, hidden_dim)
...
class _Cfg:
    ...
    output_dir  = "/work/pi_nwycoff_umass_edu/hang/auto/runs/idea002/design001"
    resume      = ""
...
def main():
    ...
    model = SapiensPose3D(
        ...
        drop_path_rate=args.drop_path, attention_method="dense",
    ).to(device)
