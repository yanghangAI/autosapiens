"""Sapiens-based RGBD pose estimation model -- design003: Feature Pyramid with 3 Scales.

Extracts layers 7, 15, 23. Projects each to 256d, concatenates, projects to 1024d.
"""
...
class SapiensBackboneRGBD(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> list[torch.Tensor]:
        """Return list of 3 feature maps from layers 7, 15, 23."""
        ...
        for feat in normed:
            B, N, C = feat.shape
            results.append(feat.transpose(1, 2).reshape(B, C, H, W))

        return results  # list of 3 tensors, each (B, 1024, H, W)
...
class FeaturePyramid(nn.Module):
    """Project each scale to proj_dim channels, concatenate, project to embed_dim."""
...
class SapiensPose3D(nn.Module):
    ...
    def __init__(self, arch="sapiens_0.3b", img_size=(640, 384), num_joints=NUM_JOINTS,
        ...
        self.aggregator = FeaturePyramid(embed_dim=embed_dim, proj_dim=256, num_scales=3)
