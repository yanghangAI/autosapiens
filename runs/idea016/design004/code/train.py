"""Training entry point — design004: 2D Heatmap + Scalar Depth + Gaussian MSE.

Same as design001 but adds an auxiliary MSE loss on predicted heatmap vs
Gaussian target centred at GT (u,v). This provides direct spatial gradient
into the heatmap logits to accelerate early training convergence.
"""
...
def train_one_epoch(model, loader, optimizer, scaler, device, epoch, args,
    ...
    for i, batch in enumerate(pbar):
        ...
        del x, out, l_xy, l_z, l_pose, l_dep, l_uv, l_hm, loss
        n += 1
        if n == 1:
...
def validate_heatmap(model, loader, device, args) -> dict:
    ...
    for batch in pbar:
        ...
        # Loss in UV+Z space (no heatmap aux during val)
