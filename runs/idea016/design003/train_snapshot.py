"""Training entry point — design003: Full 3D Volumetric Heatmap (40×24×16).

Head outputs [u_norm, v_norm, d_abs] where d_abs is absolute depth in metres.
Loss computed in normalized [u,v,d/d_scale] space. MPJPE computed after
decoding back to root-relative metres.
"""
...
def decode_joints_3d(pred_uvd, K, img_h, img_w, pelvis_abs):
    ...
    pelvis_abs = (X_world=depth, Y_world=left-right, Z_world=up-down) from infra
    GT joints = (x_rel, y_rel, z_rel) = (Y_world_rel, Z_world_rel, X_world_rel)
    i.e. joints[:,:,0] = x = Y_world direction, joints[:,:,2] = z = X_world (depth)

    Head predicts [u_norm, v_norm, d_abs] where d_abs = X_world_joint (depth).
    Recover Y_world and Z_world from unprojection, then subtract pelvis.
    """
    u_px   = pred_uvd[:, :, 0] * img_w
    v_px   = pred_uvd[:, :, 1] * img_h
    d_abs  = pred_uvd[:, :, 2]           # X_world = absolute depth
    cx = K[:, 0, 2:3]; cy = K[:, 1, 2:3]
    fx = K[:, 0, 0:1]; fy = K[:, 1, 1:2]
    # Y_world and Z_world for joints
    y_world = -(u_px - cx) * d_abs / fx   # (B, 70) — Y_world = x in root-relative
    z_world = -(v_px - cy) * d_abs / fy   # (B, 70) — Z_world = y in root-relative
    x_world = d_abs                         # (B, 70) — X_world = z in root-relative

    # pelvis_abs = (X_world_pel, Y_world_pel, Z_world_pel)
    x_world_pel = pelvis_abs[:, 0:1]   # (B, 1)
    y_world_pel = pelvis_abs[:, 1:2]   # (B, 1)
    z_world_pel = pelvis_abs[:, 2:3]   # (B, 1)

    # Root-relative in GT joint coordinate ordering: (x_rel, y_rel, z_rel)
    # x_rel = Y_world_joint - Y_world_pel
    # y_rel = Z_world_joint - Z_world_pel
    # z_rel = X_world_joint - X_world_pel
    x_rel = y_world - y_world_pel   # (B, 70)
    y_rel = z_world - z_world_pel
    z_rel = x_world - x_world_pel
...
def train_one_epoch(model, loader, optimizer, scaler, device, epoch, args,
    ...
    for i, batch in enumerate(pbar):
        ...
        # Build GT targets: [u_norm, v_norm, d_abs] for each joint
        with torch.no_grad():
            X_pel  = gt_pelvis_abs[:, 0:1].unsqueeze(1)         # (B, 1, 1) pelvis X=depth
            gt_d_abs = X_pel + joints[:, :, 2:3]                # (B, 70, 1) absolute depth
        ...
        with torch.amp.autocast("cuda", enabled=args.amp):
            out    = model(x)
            # Scale d_abs to [0,1] range to balance with UV
            d_scale = args.d_scale
            pred_uvd_scaled = out["joints"].clone()
            pred_uvd_scaled = torch.cat([
                out["joints"][:, :, :2],
                out["joints"][:, :, 2:3] / d_scale
            ], dim=-1)
            gt_uvd_scaled = torch.cat([
                gt_uvd[:, :, :2],
                gt_uvd[:, :, 2:3] / d_scale
            ], dim=-1)
        ...
        with torch.no_grad():
            pf_decoded = decode_joints_3d(out["joints"].float(), K, args.img_h, args.img_w, gt_pelvis_abs)
...
@torch.no_grad()
def validate_3d(model, loader, device, args) -> dict:
    """Validation with 3D volumetric decode: out['joints'] is (u,v,d_abs) space."""
    model.eval()
    ...
    for batch in pbar:
        ...
        # Loss in normalized UVD space
        X_pel  = gt_pelvis_abs[:, 0:1].unsqueeze(1)
        gt_d_abs = X_pel + joints[:, :, 2:3]
        X_ref  = gt_d_abs
        u_px   = K[:, 0:1, 0:1] * (-joints[:, :, 0:1]) / X_ref + K[:, 0:1, 2:3]
        v_px   = K[:, 1:2, 1:2] * (-joints[:, :, 1:2]) / X_ref + K[:, 1:2, 2:3]
        gt_uvd = torch.cat([u_px / args.img_w, v_px / args.img_h, gt_d_abs], dim=-1)
        d_scale = args.d_scale
        pred_uvd_scaled = torch.cat([out["joints"][:, :, :2], out["joints"][:, :, 2:3] / d_scale], dim=-1)
        gt_uvd_scaled   = torch.cat([gt_uvd[:, :, :2], gt_uvd[:, :, 2:3] / d_scale], dim=-1)
        l_pose = pose_loss(pred_uvd_scaled[:, BODY_IDX], gt_uvd_scaled[:, BODY_IDX]).item()
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.val_interval == 0 or (epoch + 1) == args.epochs:
            val_m = validate_3d(model, val_loader, device, args)
