"""Training entry point for SapiensPose3D — idea013/design004.

Hard-Joint-Weighted Loss + Constant Decay LLRD (gamma=0.90, unfreeze_epoch=5):
...
def train_one_epoch(model, loader, optimizer, scaler, device, epoch, args,
                    iter_logger=None, joint_weights=None, err_accum=None) -> dict:
    model.train()
    optimizer.zero_grad()
    ...
    for i, batch in enumerate(pbar):
        ...
        with torch.amp.autocast("cuda", enabled=args.amp):
            ...
            if joint_weights is not None:
                # Weighted per-joint smooth L1 loss (epochs >= 1)
        ...
        with torch.no_grad():
            ...
            # Accumulate per-joint errors during epoch 0 for weight computation
            if err_accum is not None:
                per_joint_err = (pf[:, BODY_IDX] - joints[:, BODY_IDX].float()).abs().mean(dim=(0, 2))
                err_accum["sum"] += per_joint_err
                err_accum["count"] += 1
...
def main():
    ...
    # ── Joint-weight accumulators (computed after epoch 0) ───────────────────
    joint_weights = None
    err_accum = {"sum": torch.zeros(22, device=device), "count": 0}
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        t0 = time.time()
        # Pass err_accum only during epoch 0 for weight collection
        cur_err_accum = err_accum if (epoch == 0 and joint_weights is None) else None
        train_m = train_one_epoch(model, train_loader, optimizer, scaler, device, epoch + 1, args,
                                  iter_logger=iter_logger, joint_weights=joint_weights,
                                  err_accum=cur_err_accum)

        # Compute per-joint weights after epoch 0
        if epoch == 0 and joint_weights is None and err_accum["count"] > 0:
            mean_err = err_accum["sum"] / err_accum["count"]
