...
design003: Stochastic Weight Averaging (SWA) over last 5 epochs.
Epochs 0-14: standard LLRD training.
Epochs 15-19: switch to constant LR = 0.5 * cosine_LR_at_epoch15,
              accumulate uniform average of weights at end of each epoch.
Validates on SWA model during SWA phase. No architecture or loss changes.
"""
...
def main():
    ...
    _swa_lr_scale = None  # constant LR scale for SWA phase
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if _in_swa:
            if _swa_lr_scale is None:
                # Record constant SWA LR from cosine value at swa_start_epoch
        ...
        print(f"Epoch {epoch+1}/{args.epochs}  lr_backbone={lr_bb:.2e}  lr_head={lr_hd:.2e}"
              + (f"  [SWA snapshot {_swa_n}]" if _in_swa else ""))
