...
- Train 20 epochs with EMA (decay=0.999), validate on EMA model.
- After epoch 20: load EMA weights into live model, run 1 extra epoch at
  flat LR=1e-6 (polish pass). Report polished-model val metric as final.
No architecture or loss changes from idea014/design003.
"""
...
def main():
    ...
    # First free everything we can before copying EMA weights
    del optimizer
    del scaler
    import gc
    gc.collect()
    torch.cuda.empty_cache()
    print(f"  [Polish pre-load] GPU: alloc={torch.cuda.memory_allocated(device)/1024**3:.2f}GB  reserved={torch.cuda.memory_reserved(device)/1024**3:.2f}GB")
    ...
    # Free EMA model to reclaim GPU memory (optimizer/scaler already freed above)
    del ema_model
    gc.collect()
    torch.cuda.empty_cache()
    print(f"  [Polish] GPU after del: alloc={torch.cuda.memory_allocated(device)/1024**3:.2f}GB  reserved={torch.cuda.memory_reserved(device)/1024**3:.2f}GB")
    ...
    # Re-open iter_logger for the polish epoch (appending)
    iter_logger_polish = IterLogger(str(out_dir / "iter_metrics_polish.csv"))
    ...
    # Append polish epoch row to metrics.csv
    _csv_fieldnames = [
    ...
    with open(str(out_dir / "metrics.csv"), "a", newline="") as _f:
        _writer = csv.DictWriter(_f, fieldnames=_csv_fieldnames, extrasaction="ignore")
        _writer.writerow({
            "epoch": args.epochs + 1,
            "lr_backbone": args.polish_lr,
    ...
    print(f"\nTraining + polish complete. Best val weighted MPJPE = {best_mpjpe:.1f}mm")
    print(f"  Polish val body = {final_val_body:.1f}mm")
