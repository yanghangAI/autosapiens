"""Sapiens-based RGBD pose estimation model — design001: discretized_depth_pe.

Replaces the standard 2D positional embedding in the ViT backbone with a
decomposed row + column + depth bucket positional embedding.
"""
...
class DepthBucketPE(nn.Module):
    ...
    def forward(self, patch_tokens: torch.Tensor, depth_ch: torch.Tensor) -> torch.Tensor:
        ...
        # Depth buckets via average pooling to patch grid
        depth_patches = F.avg_pool2d(depth_ch, kernel_size=16, stride=16)  # (B, 1, h, w)
        depth_bins = (depth_patches.squeeze(1) * self.num_depth_bins).long()  # (B, h, w)
        depth_bins = depth_bins.clamp(0, self.num_depth_bins - 1)              # (B, h, w)
        depth_bins_flat = depth_bins.reshape(B, -1)                            # (B, N)

        # Compose PE: (1, N, D) + (1, N, D) + (B, N, D)
        pe = (self.row_emb[rows].unsqueeze(0)        # (1, N, D)
              + self.col_emb[cols].unsqueeze(0)       # (1, N, D)
              + self.depth_emb[depth_bins_flat])      # (B, N, D)
