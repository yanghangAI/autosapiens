...
class DepthProjector(nn.Module):
    """Lightweight depth-to-token projection for mid-fusion injection.

    Maps a (B, 1, img_h, img_w) depth map to a (B, N_tokens, embed_dim)
    additive token bias using a single Conv2d patch embedding + LayerNorm.
    Both Conv2d weight and bias are zero-initialised so the model starts as
    a pure-RGB model at epoch 0.
    """

    def __init__(self, patch_size: int = 16, embed_dim: int = 1024,
                 img_h: int = 640, img_w: int = 384):
        super().__init__()
        # padding=2 matches RGB patch embed -> spatial output (40, 24)
        self.depth_patch_embed = nn.Conv2d(
            1, embed_dim, kernel_size=patch_size, stride=patch_size, padding=2,
        )
        self.depth_norm = nn.LayerNorm(embed_dim)
        # Zero-init: depth bias is all-zero at epoch 0 (pure-RGB start)
        nn.init.zeros_(self.depth_patch_embed.weight)
        nn.init.zeros_(self.depth_patch_embed.bias)

    def forward(self, depth: torch.Tensor) -> torch.Tensor:
        # depth: (B, 1, H, W)
        x = self.depth_patch_embed(depth)   # (B, embed_dim, 40, 24)
        x = x.flatten(2).transpose(1, 2)    # (B, 960, embed_dim)
        x = self.depth_norm(x)              # (B, 960, embed_dim)
        return x
...
class SapiensBackboneMidFusion(nn.Module):
    """Sapiens ViT with mid-layer depth injection (mid_fusion strategy).

    RGB is processed through the standard 3-channel patch embed. After block
    11 (0-indexed), depth tokens produced by DepthProjector are added
    additively to the token sequence before block 12 executes.
    """

    INJECT_BEFORE = 12   # inject before block 12 (i.e. after block 11)
    ...
    def __init__(self, arch="sapiens_0.3b", img_size=(640, 384), drop_path_rate=0.0):
        ...
        self.depth_proj = DepthProjector(
            patch_size=16, embed_dim=self.embed_dim,
            img_h=img_size[0], img_w=img_size[1],
        )

    def forward(self, rgb: torch.Tensor, depth: torch.Tensor) -> torch.Tensor:
        # rgb:   (B, 3, H, W)
        # depth: (B, 1, H, W)
        # Replicate VisionTransformer.forward manually to inject depth at block 12.
        B = rgb.size(0)
        vit = self.vit

        # Patch embedding: returns (tokens, patch_resolution)
        x, patch_resolution = vit.patch_embed(rgb)   # x: (B, N, C)

        # No cls_token (with_cls_token=False)

        # Positional embedding (resize if needed)
        x = x + resize_pos_embed(
            vit.pos_embed,
            vit.patch_resolution,
            patch_resolution,
            mode=vit.interpolate_mode,
            num_extra_tokens=vit.num_extra_tokens,
        )
        x = vit.drop_after_pos(x)
        x = vit.pre_norm(x)

        n_layers = len(vit.layers)
        for i, block in enumerate(vit.layers):
            if i == self.INJECT_BEFORE:            # inject before block 12
                d = self.depth_proj(depth)          # (B, N, embed_dim)
                x = x + d
            x = block(x)
            # Apply final layer norm at the last block (mirrors VisionTransformer.forward)
            if i == n_layers - 1 and vit.final_norm:
                x = vit.ln1(x)

        # x: (B, N, embed_dim) — no cls_token so N = n_h * n_w = 960
...
def load_sapiens_pretrained(model: nn.Module, ckpt_path: str, verbose: bool = True) -> None:
    """Load Sapiens RGB pretrain checkpoint into SapiensBackboneMidFusion.

    The patch embed is 3-channel in this design, so weights load directly
    without any channel expansion. DepthProjector weights are zero-initialised
    and intentionally not loaded from the checkpoint.
    """
    ...
    for k, v_model in model_sd.items():
        # Skip DepthProjector — zero-init is intentional
        if k.startswith("backbone.depth_proj."):
            continue
    ...
    if verbose:
        ...
        print(f"[weights] DepthProjector: zero-init (not loaded from checkpoint)")
...
class SapiensPose3D(nn.Module):
    """Backbone + Head wrapper for mid-fusion.

    Accepts a concatenated (B, 4, H, W) tensor (rgb channels 0:3, depth
    channel 3:4) and splits it before passing to SapiensBackboneMidFusion.
    ...
    def __init__(self, arch="sapiens_0.3b", img_size=(640, 384), num_joints=NUM_JOINTS,
        ...
        super().__init__()
        self.backbone = SapiensBackboneMidFusion(arch=arch, img_size=img_size,
...
class _Cfg:
    ...
    output_dir  = "/work/pi_nwycoff_umass_edu/hang/auto/runs/idea001/design002"
    resume      = ""
...
def main():
    ...
    # Optimizer — 3 groups: ViT backbone, DepthProjector, Head
    # DepthProjector uses lr_head (1e-4) as a new module needing faster learning
    optimizer = torch.optim.AdamW(
        [{"params": model.backbone.vit.parameters(),        "lr": args.lr_backbone},
         {"params": model.backbone.depth_proj.parameters(), "lr": args.lr_head},
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        print(f"Epoch {epoch+1}/{args.epochs}  lr_backbone={lr_bb:.2e}  lr_depth_proj={optimizer.param_groups[1]['lr']:.2e}  lr_head={lr_hd:.2e}")
