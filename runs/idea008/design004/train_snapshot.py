...
def main():
    ...
    model = SapiensPose3D(
        ...
        num_fine_depth_bins=args.num_fine_depth_bins,
        num_coarse_depth_bins=args.num_coarse_depth_bins,
    ).to(device)
    if args.pretrain and not args.resume:
        model.load_pretrained(args.pretrain)
    print(f"  Parameters: {sum(p.numel() for p in model.parameters()) / 1e6:.1f}M")

    # Optimizer — 3 groups: backbone ViT, depth_bucket_pe, head
    depth_pe_ids = {id(p) for p in model.backbone.depth_bucket_pe.parameters()}
    backbone_params = [p for p in model.backbone.parameters()
                       if id(p) not in depth_pe_ids]
