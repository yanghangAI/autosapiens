"""Sapiens-based RGBD pose estimation model — design004: hybrid_depth_pe.

Replaces the standard 2D positional embedding in the ViT backbone with a
decomposed row + column + hybrid fine/coarse depth positional embedding.
"""
...
class HybridDepthPE(nn.Module):
    ...
    Parameters:
        ...
        fine_depth_emb   : (num_fine_bins, embed_dim) — zero-initialized
        coarse_depth_emb : (num_coarse_bins, embed_dim) — zero-initialized
    """
    ...
    def __init__(
        ...
        w_tok: int,
        embed_dim: int,
        num_fine_depth_bins: int = 16,
        num_coarse_depth_bins: int = 4,
    ):
        super().__init__()
        self.h_tok = h_tok
        self.w_tok = w_tok
        self.num_fine_depth_bins = num_fine_depth_bins
        self.num_coarse_depth_bins = num_coarse_depth_bins

        self.row_emb = nn.Parameter(torch.zeros(h_tok, embed_dim))
        self.col_emb = nn.Parameter(torch.zeros(w_tok, embed_dim))
        self.fine_depth_emb = nn.Parameter(torch.zeros(num_fine_depth_bins, embed_dim))
        self.coarse_depth_emb = nn.Parameter(torch.zeros(num_coarse_depth_bins, embed_dim))
    ...
    def forward(self, patch_tokens: torch.Tensor, depth_ch: torch.Tensor) -> torch.Tensor:
        ...
        # Fine local depth encoding via average pooling to patch grid
        depth_patches = F.avg_pool2d(depth_ch, kernel_size=16, stride=16)  # (B, 1, h, w)
        depth_pos = depth_patches.squeeze(1).clamp(0.0, 1.0) * (self.num_fine_depth_bins - 1)
        idx_lo = torch.floor(depth_pos).long()
        idx_hi = torch.clamp(idx_lo + 1, max=self.num_fine_depth_bins - 1)
        alpha = (depth_pos - idx_lo.float()).reshape(B, -1, 1)
        fine_lo = self.fine_depth_emb[idx_lo.reshape(B, -1)]
        fine_hi = self.fine_depth_emb[idx_hi.reshape(B, -1)]
        fine_depth_pe = (1.0 - alpha) * fine_lo + alpha * fine_hi

        # Coarse global depth encoding as a single broadcast code per image
        global_depth = depth_patches.mean(dim=(1, 2, 3)).clamp(0.0, 1.0)
        global_pos = global_depth * (self.num_coarse_depth_bins - 1)
        g_lo = torch.floor(global_pos).long()
        g_hi = torch.clamp(g_lo + 1, max=self.num_coarse_depth_bins - 1)
        g_alpha = (global_pos - g_lo.float()).unsqueeze(-1)
        coarse_depth_pe = (
            (1.0 - g_alpha) * self.coarse_depth_emb[g_lo]
            + g_alpha * self.coarse_depth_emb[g_hi]
        ).unsqueeze(1).expand(B, h * w, -1)

        pe = (
            self.row_emb[rows].unsqueeze(0)
            + self.col_emb[cols].unsqueeze(0)
            + fine_depth_pe
            + coarse_depth_pe
        )
...
class SapiensBackboneRGBD(nn.Module):
    """Sapiens ViT with 4-channel patch embed + hybrid depth PE."""
    ...
    def __init__(
        ...
        self.depth_bucket_pe = HybridDepthPE(
            h_tok,
            w_tok,
            self.embed_dim,
            num_fine_depth_bins=num_fine_depth_bins,
            num_coarse_depth_bins=num_coarse_depth_bins,
        )
...
def load_sapiens_pretrained(model: nn.Module, ckpt_path: str, verbose: bool = True) -> None:
    ...
    - Interpolates pos_embed, then uses it to initialize HybridDepthPE
    ...
    # 3. Initialize HybridDepthPE from pretrained 2D pos_embed
    if pretrained_pe_2d is not None:
        ...
        with torch.no_grad():
            ...
            # fine_depth_emb and coarse_depth_emb stay zero-initialized
        if verbose:
            print(
                "[weights] HybridDepthPE: row_emb & col_emb init from pretrained 2D PE; "
                "fine_depth_emb and coarse_depth_emb zero-initialized"
            )
...
class SapiensPose3D(nn.Module):
    ...
    def __init__(self, arch="sapiens_0.3b", img_size=(640, 384), num_joints=NUM_JOINTS,
                 ...
                 num_fine_depth_bins=16, num_coarse_depth_bins=4):
        super().__init__()
