"""Sapiens-based RGBD pose estimation model — design002: gated_continuous_depth_pe.

Replaces the standard 2D positional embedding in the ViT backbone with a
decomposed row + column + continuously interpolated, gated depth positional embedding.
"""
...
class DepthBucketPE(nn.Module):
    """Decomposed row + column + gated continuous depth positional embedding.
    ...
    Parameters:
        ...
        depth_emb: (num_depth_bins, embed_dim) — zero-initialized
        depth_gate: scalar sigmoid gate on the interpolated depth term
    """
    ...
    def forward(self, patch_tokens: torch.Tensor, depth_ch: torch.Tensor) -> torch.Tensor:
        ...
        # Average-pool depth to the patch grid, then interpolate between
        # the two nearest learned depth anchors and gate the result.
        depth_patches = F.avg_pool2d(depth_ch, kernel_size=16, stride=16)  # (B, 1, h, w)
        depth_pos = depth_patches.squeeze(1).clamp(0.0, 1.0) * (self.num_depth_bins - 1)
        ...
        depth_pe = torch.sigmoid(self.depth_gate) * depth_pe
