"""Sapiens-based RGBD pose estimation model — design001: continuous_depth_pe.

Replaces the standard 2D positional embedding in the ViT backbone with a
decomposed row + column + continuously interpolated depth positional embedding.
"""
...
class DepthBucketPE(nn.Module):
    """Decomposed row + column + continuously interpolated depth PE.
    ...
    def forward(self, patch_tokens: torch.Tensor, depth_ch: torch.Tensor) -> torch.Tensor:
        ...
        # Average-pool normalized depth to the patch grid, then interpolate
        # between the two nearest learned depth anchors.
