"""Training entry point for SapiensPose3D — idea012/design004.

R-Drop consistency regularization: adds MSE between two stochastic forward
passes (different dropout/drop_path masks) on body joints. Second pass is
...
def train_one_epoch(model, loader, optimizer, scaler, device, epoch, args,
    ...
    for i, batch in enumerate(pbar):
        ...
        with torch.amp.autocast("cuda", enabled=args.amp):
            ...
            l_task = l_pose + args.lambda_depth * l_dep + args.lambda_uv * l_uv

            # R-Drop: second forward pass with different dropout/drop_path masks
            with torch.no_grad():
                out2 = model(x)
            pred1_body = out["joints"][:, BODY_IDX, :]          # (B, 22, 3)
            pred2_body = out2["joints"][:, BODY_IDX, :].detach()  # (B, 22, 3)
            l_consist  = F.mse_loss(pred1_body, pred2_body)

            loss = (l_task + args.rdrop_alpha * l_consist) / args.accum_steps
        ...
        del x, out, out2, l_pose, l_dep, l_uv, l_task, l_consist, loss
        n += 1
        if n == 1:
