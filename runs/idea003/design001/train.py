...
class _Cfg:
    ...
    output_dir  = "/work/pi_nwycoff_umass_edu/hang/auto/runs/idea003/design001"
    resume      = ""
...
def train_one_epoch(model, loader, optimizer, scaler, device, epoch, args,
    ...
    for i, batch in enumerate(pbar):
        ...
        with torch.amp.autocast("cuda", enabled=args.amp):
            ...
            # DESIGN 001: Homoscedastic uncertainty weighting
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        w_pose  = torch.exp(-model.module.log_var_pose if hasattr(model, 'module') else -model.log_var_pose).item()
        w_depth = torch.exp(-model.module.log_var_depth if hasattr(model, 'module') else -model.log_var_depth).item()
        w_uv    = torch.exp(-model.module.log_var_uv if hasattr(model, 'module') else -model.log_var_uv).item()
        print(f"  ({epoch_time:.0f}s)\n  → learned weights: pose={w_pose:.3f}  depth={w_depth:.3f}  uv={w_uv:.3f}\n")

        logger.log({"epoch": epoch + 1, "lr_backbone": lr_bb, "lr_head": lr_hd,
                    "w_pose": w_pose, "w_depth": w_depth, "w_uv": w_uv,
