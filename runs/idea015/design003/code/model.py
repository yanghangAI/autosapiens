...
class Pose3DHead(nn.Module):
    """Transformer decoder head with three-pass shared-decoder refinement (progressive deep supervision)."""
    ...
    def __init__(self, in_channels, num_joints=NUM_JOINTS, hidden_dim=256,
        ...
        # Shared refinement MLP (used for both J1->J2 and J2->J3 transitions)
        self.refine_mlp = nn.Sequential(
            nn.Linear(3, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, hidden_dim),
        )
        # Additional output heads for pass 2 and pass 3
        self.joints_out2 = nn.Linear(hidden_dim, 3)
        self.joints_out3 = nn.Linear(hidden_dim, 3)

        self._init_weights()

    def _init_weights(self):
        nn.init.trunc_normal_(self.joint_queries.weight, std=0.02)
        for m in [self.joints_out, self.depth_out, self.uv_out, self.joints_out2, self.joints_out3]:
    ...
    def forward(self, feat: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        # Pass 2: inject J1 via shared refine_mlp
        R1       = self.refine_mlp(J1)         # (B, 70, hidden_dim)
        queries2 = out1 + R1                   # (B, 70, hidden_dim)
        out2     = self.decoder(queries2, memory)  # (B, 70, hidden_dim)
        J2       = self.joints_out2(out2)      # (B, 70, 3)

        # Pass 3: inject J2 via same shared refine_mlp
        R2       = self.refine_mlp(J2)         # (B, 70, hidden_dim)
        queries3 = out2 + R2                   # (B, 70, hidden_dim)
        out3     = self.decoder(queries3, memory)  # (B, 70, hidden_dim)
