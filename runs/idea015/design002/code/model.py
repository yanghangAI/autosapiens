...
class SapiensBackboneRGBD(nn.Module):
    """Sapiens ViT with 4-channel patch embed + decomposed depth bucket PE."""
...
class Pose3DHead(nn.Module):
    """Transformer decoder head with two-pass shared-decoder refinement (Gaussian cross-attention bias)."""
    ...
    def __init__(self, in_channels, num_joints=NUM_JOINTS, hidden_dim=256,
        ...
        self.H_tok = 40   # feature map height tokens (640 / 16)
        self.W_tok = 24   # feature map width tokens (384 / 16)
        ...
        # Learnable scalar for Gaussian bias scale (init=0 → no bias at step 0)
        self.attn_bias_scale = nn.Parameter(torch.zeros(1))
        # Second output head for refined prediction
        self.joints_out2 = nn.Linear(hidden_dim, 3)

        self._init_weights()

    def _init_weights(self):
        nn.init.trunc_normal_(self.joint_queries.weight, std=0.02)
        for m in [self.joints_out, self.depth_out, self.uv_out, self.joints_out2]:
    ...
    def forward(self, feat: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        # Pass 1: standard single-pass (same as baseline)
        out1 = self.decoder(queries, memory)   # (B, 70, hidden_dim)
        J1   = self.joints_out(out1)           # (B, 70, 3)

        # Get pelvis UV from pass 1 for root-anchored projection
        pelvis_uv_pred = self.uv_out(out1[:, 0, :])  # (B, 2) in [0,1]

        # Compute per-joint UV coords projected onto memory grid
        z_clamped = J1[:, :, 2:3].clamp(min=0.1)                            # (B, 70, 1)
        uv_norm   = pelvis_uv_pred.unsqueeze(1) + 0.5 * (J1[:, :, :2] / z_clamped)  # (B, 70, 2)
        uv_norm   = uv_norm.clamp(0.0, 1.0)

        # Convert to grid pixel coords
        u_grid = uv_norm[:, :, 0] * (W_tok - 1)  # (B, 70)
        v_grid = uv_norm[:, :, 1] * (H_tok - 1)  # (B, 70)

        # Precompute patch grid coordinates: (1, 1, 960, 2)
        rows = torch.arange(H_tok, device=device).unsqueeze(1).expand(H_tok, W_tok).reshape(-1)  # (960,)
        cols = torch.arange(W_tok, device=device).unsqueeze(0).expand(H_tok, W_tok).reshape(-1)  # (960,)
        grid = torch.stack([rows, cols], dim=-1).float().unsqueeze(0).unsqueeze(0)  # (1, 1, 960, 2)

        # Joint projected positions: (B, 70, 1, 2)
        mu = torch.stack([v_grid, u_grid], dim=-1).unsqueeze(2)  # (B, 70, 1, 2)

        # Gaussian bias (log-Gaussian, additive to logits): sigma=2.0 patches
        sigma = 2.0
        dist2 = ((grid - mu) ** 2).sum(dim=-1)           # (B, 70, 960)
        gauss_bias = -dist2 / (2 * sigma ** 2)            # (B, 70, 960)

        # Scale by learnable scalar (init=0 → bias=0 at step 0)
        gauss_bias = gauss_bias * self.attn_bias_scale    # (B, 70, 960)
        gauss_bias = gauss_bias.clamp(min=-1e4)           # avoid -inf

        # Expand over heads: (B*num_heads, 70, 960)
        attn_bias_expanded = gauss_bias.unsqueeze(1).expand(-1, self.num_heads, -1, -1)
        attn_bias_expanded = attn_bias_expanded.reshape(B * self.num_heads, self.num_joints, H_tok * W_tok)

        # Pass 2: manual layer loop with Gaussian bias on cross-attention (norm_first=True)
        tgt = queries  # re-start from original queries
        for layer in self.decoder.layers:
            # Self-attention (norm first)
            tgt = tgt + layer.dropout1(layer.self_attn(layer.norm1(tgt), layer.norm1(tgt), layer.norm1(tgt))[0])
            # Cross-attention with Gaussian bias (norm first)
