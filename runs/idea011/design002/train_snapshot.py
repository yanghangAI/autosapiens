...
def main():
    ...
    depth_pe_names = {"backbone.depth_bucket_pe." + n for n, _ in model.backbone.depth_bucket_pe.named_parameters()}
    depth_pe_params = list(model.backbone.depth_bucket_pe.parameters())
    depth_pe_ids = {id(p) for p in depth_pe_params}
    ...
    def _build_optimizer_frozen():
        ...
        # Freeze blocks 0-11, patch_embed, pos_embed
        for p in vit.patch_embed.parameters():
            p.requires_grad = False
        # pos_embed is a buffer (not Parameter) — no action needed
        for i in range(12):
            for p in vit.layers[i].parameters():
                p.requires_grad = False
        # Unfreeze blocks 12-23
        for i in range(12, NUM_BLOCKS):
            for p in vit.layers[i].parameters():
                p.requires_grad = True
        # depth_pe always unfrozen
        for p in depth_pe_params:
            p.requires_grad = True
        # head always unfrozen
    ...
    def _build_optimizer_full():
        ...
        for p in model.parameters():
            p.requires_grad = True
        # pos_embed is a buffer — skip

        groups = []
        # Embed group (patch_embed params)
        embed_params = list(vit.patch_embed.parameters())
        # Note: vit.pos_embed is a buffer, not in parameters()
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Report LR: deepest block is last block group
        ...
        if epoch < UNFREEZE_EPOCH:
            lr_bb = optimizer.param_groups[11]["lr"]   # block 23 (index 11 in frozen phase)
            lr_hd = optimizer.param_groups[13]["lr"]   # head
        else:
            lr_bb = optimizer.param_groups[24]["lr"]   # block 23 (index 24 in full phase)
