...
# 6 symmetric limb segment pairs (left_a, left_b, right_a, right_b)
SYM_PAIRS = [
...
def bone_length_loss(pred_joints, gt_joints, edges):
    """Mean absolute bone-length deviation between pred and GT."""
    total = 0.0
    for (a, b) in edges:
...
def symmetry_loss(pred_joints, sym_pairs):
    """Mean absolute left-right bone-length asymmetry."""
    total = 0.0
...
def train_one_epoch(model, loader, optimizer, scaler, device, epoch, args,
    ...
    for i, batch in enumerate(pbar):
        ...
        del x, out, l_pose1, l_pose2, l_bone, l_sym, l_pose, l_dep, l_uv, loss
        n += 1
        if n == 1:
