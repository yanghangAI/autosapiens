...
class Pose3DHead(nn.Module):
    """Transformer decoder head with two-pass shared-decoder refinement (query injection)."""
    ...
    def __init__(self, in_channels, num_joints=NUM_JOINTS, hidden_dim=256,
        ...
        # Anatomical group embeddings for pass 2 initialization
        # Group 0: torso (pelvis + spine + neck + head + eyes)
        # Group 1: arms (shoulders, elbows, wrists, hands within 0-21 range)
        # Group 2: legs (hips, knees, ankles, feet within 0-21 range)
        # Group 3: extremities (index 22+, i.e. hands detail, face, etc.)
        ...
        for j in _LEGS:
            if j < num_joints:
                joint_group_ids[j] = 2
        # All indices 22+ not in _TORSO → group 3
        for j in range(22, num_joints):
            if j not in _TORSO:
                joint_group_ids[j] = 3
        self.register_buffer("joint_group_ids", joint_group_ids)  # (70,) fixed buffer

        # Learnable group embeddings, zero-initialized so training starts identical to baseline
    ...
    def forward(self, feat: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        # Pass 2: refined prediction (shared decoder weights)
        out2 = self.decoder(queries2, memory)  # (B, 70, hidden_dim)
