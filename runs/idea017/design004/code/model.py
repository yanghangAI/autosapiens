...
class Pose3DHead(nn.Module):
    ...
    def forward(self, feat_t: torch.Tensor,
                feats_context: list | None = None) -> dict[str, torch.Tensor]:
        """Accept centre-frame features and optional list of context-frame features.

        feats_context: list of feature tensors from context frames [prev, next].
        Memory ordering: [prev_mem, t_mem, next_mem] for symmetric temporal window.

        mems[0] = mem_t (centre), mems[1] = mem_prev, mems[2] = mem_next
        Final concat: [mems[1], mems[0], mems[2]] = [prev, t, next]
        """
        B = feat_t.size(0)
        # Project centre frame features
        mem_t = self.input_proj(feat_t.flatten(2).transpose(1, 2))   # (B, 960, hidden)
        mems = [mem_t]  # mems[0] = centre frame

        if feats_context:
            for f in feats_context:
                # Each context frame projected with the same shared input_proj
                mems.append(self.input_proj(f.flatten(2).transpose(1, 2)))  # (B, 960, hidden)

        # Build memory with symmetric ordering [prev, t, next]
        if len(mems) == 3:
            # mems[1]=prev, mems[0]=centre, mems[2]=next → [prev, t, next]
            memory = torch.cat([mems[1], mems[0], mems[2]], dim=1)  # (B, 2880, hidden)
        elif len(mems) == 2:
            # Only one context frame: [context, t]
            memory = torch.cat([mems[1], mems[0]], dim=1)            # (B, 1920, hidden)
        else:
            memory = mems[0]                                          # (B, 960, hidden)
...
class SapiensPose3D(nn.Module):
    ...
    def forward(self, x_t: torch.Tensor,
                ...
                x_next: torch.Tensor | None = None) -> dict[str, torch.Tensor]:
        """Three-frame symmetric temporal fusion.

        Centre-frame backbone: full gradient (LLRD).
        Past + future-frame backbones: frozen inference inside torch.no_grad().
        Context features are collected and passed to head as feats_context list.
        """
        ...
        if x_prev is not None:
            ...
            feats_context.append(feat_prev.detach())

        if x_next is not None:
            with torch.no_grad():
                feat_next = self.backbone(x_next)
            feats_context.append(feat_next.detach())

        return self.head(feat_t, feats_context if feats_context else None)
