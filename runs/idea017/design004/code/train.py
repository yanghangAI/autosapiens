"""Training entry point — design004: Three-Frame Symmetric Temporal Fusion.

Three 4-channel frames (x_t, x_prev, x_next) are fed to the shared backbone.
Past and future backbone passes run inside torch.no_grad() (frozen context).
Only the centre-frame backbone participates in gradient updates and LLRD.
The three projected feature maps are concatenated: memory = [prev, t, next]
→ (B, 2880, 384). Only centre-frame joints are supervised.
Validation uses single-frame mode (x_prev=None, x_next=None).
"""
...
def _load_aux_frame(dataset, cached, npy_path, npz_path, label_path, aux_idx, fallback_rgb):
    """Load rgb + depth for an auxiliary frame index."""
    try:
        rgb_aux = dataset._read_frame(
            cached["folder_name"], cached["seq_name"], aux_idx, label_path
        )
    except Exception:
        rgb_aux = fallback_rgb.copy()

    depth_aux = dataset._read_depth(npy_path, npz_path, aux_idx, label_path)
    if depth_aux is None:
        depth_aux = np.zeros(fallback_rgb.shape[:2], dtype=np.float32)

    return rgb_aux, depth_aux


class ThreeFrameBedlamDataset(BedlamFrameDataset):
    """Three-frame dataset: past (t-1), centre (t), future (t+1) in dataset-index space.

    Extra batch keys: rgb_prev, depth_prev, rgb_next, depth_next (all tensors).
    """
    ...
    def __getitem__(self, idx):
        ...
        future_idx = min(n_frames - 1, frame_idx + 1)
        ...
        rgb_prev_raw, depth_prev_raw = _load_aux_frame(
            self, cached, npy_path, npz_path, label_path, past_idx, sample["rgb"]
        )
        rgb_next_raw, depth_next_raw = _load_aux_frame(
            self, cached, npy_path, npz_path, label_path, future_idx, sample["rgb"]
        )

        sample["_rgb_prev_raw"]   = rgb_prev_raw
        sample["_depth_prev_raw"] = depth_prev_raw
        sample["_rgb_next_raw"]   = rgb_next_raw
        sample["_depth_next_raw"] = depth_next_raw
        ...
        for prefix in ("prev", "next"):
            rgb_raw = sample.pop(f"_rgb_{prefix}_raw")
            dep_raw = sample.pop(f"_depth_{prefix}_raw")
            ...
            sample[f"rgb_{prefix}"]   = _normalise_rgb(rgb_crop, self._rgb_mean, self._rgb_std)
            sample[f"depth_{prefix}"] = _normalise_depth(depth_crop, self._depth_max)

        return sample


def build_three_frame_dataloader(seq_paths, data_root, transform, batch_size,
                                  shuffle, num_workers):
    dataset = ThreeFrameBedlamDataset(seq_paths=seq_paths, data_root=data_root,
...
def train_one_epoch(model, loader, optimizer, scaler, device, epoch, args,
    ...
    for i, batch in enumerate(pbar):
        ...
        x_next = torch.cat([rgb_next, depth_next], dim=1)  # (B, 4, H, W)
        ...
        del x_t, x_prev, x_next, out, l_pose, l_dep, l_uv, loss
        n += 1
        if n == 1:
...
def main():
    ...
    print(f"Temporal mode: {args.temporal_mode}  (3-frame symmetric, past+future frozen)")
    ...
    train_loader = build_three_frame_dataloader(train_seqs, args.data_root, train_tf,
    ...
    def _val_forward(x4: torch.Tensor) -> dict:
        return original_forward(x4, None, None)
