...
class Pose3DHead(nn.Module):
    ...
    def forward(self, feat_t: torch.Tensor,
                feat_prev: torch.Tensor | None = None) -> dict[str, torch.Tensor]:
        """Accept centre-frame features and optional past-frame features.

        If feat_prev is provided, both feature maps are projected through the
        shared input_proj and concatenated along the token axis:
          memory = cat([proj(feat_prev), proj(feat_t)], dim=1)  → (B, 1920, 384)
        Otherwise (single-frame fallback for validation):
          memory = proj(feat_t)                                  → (B, 960, 384)
        """
        B = feat_t.size(0)
        mem_t = self.input_proj(feat_t.flatten(2).transpose(1, 2))   # (B, 960, hidden)
        if feat_prev is not None:
            mem_prev = self.input_proj(feat_prev.flatten(2).transpose(1, 2))  # (B, 960, hidden)
            # Concat: past frame first, then centre frame
            memory = torch.cat([mem_prev, mem_t], dim=1)               # (B, 1920, hidden)
        else:
...
class SapiensPose3D(nn.Module):
    ...
    def forward(self, x_t: torch.Tensor,
                x_prev: torch.Tensor | None = None) -> dict[str, torch.Tensor]:
        """Two-frame forward with gradient checkpointing on both backbone passes.

        Both backbone forwards are checkpointed (use_reentrant=False) to halve
        activation memory. The head then cross-attends to the concatenated memory.
        When x_prev is None (validation), falls back to single-frame mode.
        """
