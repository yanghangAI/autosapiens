...
class Pose3DHead(nn.Module):
    ...
    def forward(self, feat_t: torch.Tensor,
        ...
        If feat_prev is provided (same shared input_proj), concatenate memories:
          memory = cat([proj(feat_prev), proj(feat_t)], dim=1) → (B, 1920, hidden)
...
class SapiensPose3D(nn.Module):
    ...
    def forward(self, x_t: torch.Tensor,
        ...
        Past-frame backbone: frozen inference inside torch.no_grad().

        This halves gradient memory vs. design002 by eliminating the backward
        pass through the past-frame path.
        """
        ...
        if x_prev is not None:
            ...
            feat_prev = feat_prev.detach()  # explicitly detach (belt-and-suspenders)
        else:
            feat_prev = None
