...
def _set_backbone_frozen(model) -> None:
...
def _set_backbone_unfrozen(model) -> None:
    vit = model.backbone.vit
    for p in vit.patch_embed.parameters():
        p.requires_grad = True
...
def _build_optimizer_frozen(model, args):
    _set_backbone_frozen(model)
    for p in model.backbone.depth_bucket_pe.parameters():
        p.requires_grad = True
    for p in model.head.parameters():
        p.requires_grad = True

    groups = [{
        "params": list(model.backbone.depth_bucket_pe.parameters()),
        "lr": args.lr_depth_pe,
        "initial_lr": args.lr_depth_pe,
        "group_name": "depth_bucket_pe",
    }]
    for i in range(12, NUM_BLOCKS):
        lr = _block_lr(i, args.lr_backbone, args.gamma)
        groups.append({
            "params": list(model.backbone.vit.layers[i].parameters()),
            "lr": lr,
            "initial_lr": lr,
            "group_name": f"block{i}",
        })
    groups.append({
        "params": list(model.head.parameters()),
        "lr": args.lr_head,
        "initial_lr": args.lr_head,
        "group_name": "head",
    })
    return torch.optim.AdamW(groups, weight_decay=args.weight_decay)


def _build_optimizer_full(model, args):
    _set_backbone_unfrozen(model)
    for p in model.backbone.depth_bucket_pe.parameters():
        p.requires_grad = True
    for p in model.head.parameters():
        p.requires_grad = True
    ...
    groups = [{
        "params": list(model.backbone.vit.patch_embed.parameters()),
        "lr": _embed_lr(args.lr_backbone, args.gamma),
        "initial_lr": _embed_lr(args.lr_backbone, args.gamma),
...
def _group_lr(optimizer, group_name: str) -> float:
    for group in optimizer.param_groups:
        if group.get("group_name") == group_name:
            return group["lr"]
    raise KeyError(f"Optimizer group '{group_name}' not found")
...
def main():
    ...
    optimizer = _build_optimizer_frozen(model, args)
    ...
    for epoch in range(start_epoch, args.epochs):
        if epoch == args.unfreeze_epoch:
            print(f"\n[LLRD] Epoch {epoch}: unfreezing full backbone and rebuilding optimizer.")
            optimizer = _build_optimizer_full(model, args)

        scale = get_lr_scale(epoch, args.epochs, args.warmup_epochs)
        for g in optimizer.param_groups:
            g["lr"] = g["initial_lr"] * scale
        lr_bb = _group_lr(optimizer, "block23")
        lr_hd = _group_lr(optimizer, "head")
