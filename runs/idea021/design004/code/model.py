...
from infra import NUM_JOINTS, SAPIENS_ARCHS, _interp_pos_embed, SMPLX_SKELETON
...
class Pose3DHead(nn.Module):
    """Transformer decoder head with two-pass independent refinement decoder."""
    ...
    def __init__(self, in_channels, num_joints=NUM_JOINTS, hidden_dim=256,
        ...
        # Refinement MLP: projects coarse J1 back into query space
        self.refine_mlp = nn.Sequential(
            nn.Linear(3, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, hidden_dim),
        )
        # Independent 2-layer refinement decoder (separate weights from coarse decoder)
        ...
        # Kinematic soft-attention bias for refine decoder (design A1)
        self.register_buffer("kin_bias", _compute_kin_bias(SMPLX_SKELETON, num_joints))
        self.kin_bias_scale = nn.Parameter(torch.zeros(1))

        # Joint-group query injection before refine decoder (design A2)
    ...
    def _init_weights(self):
        ...
        # Initialize refine_mlp with small weights
        for layer in self.refine_mlp:
    ...
    def forward(self, feat: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        # Pass 1: coarse prediction via existing 4-layer decoder
