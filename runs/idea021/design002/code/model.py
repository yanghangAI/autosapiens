...
class Pose3DHead(nn.Module):
    ...
    def forward(self, feat: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        # Add group embedding to queries2 before refine decoder
        group_bias = self.group_emb(self.joint_group_ids)   # (70, hidden_dim)
        queries2   = queries2 + group_bias.unsqueeze(0)     # (B, 70, hidden_dim)

        # Pass 2: refine via independent 2-layer decoder
        out2 = self.refine_decoder(queries2, memory)  # (B, 70, hidden_dim)
